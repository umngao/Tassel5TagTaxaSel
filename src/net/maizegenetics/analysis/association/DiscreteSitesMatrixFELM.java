...
public class DiscreteSitesMatrixFELM extends AbstractFixedEffectLM {
	...
	protected void analyzeSite(int siteNumber, ArrayList<ModelEffect> baseModel) {
		// TODO implement
		byte[] geno = myGenoPheno.genotypeTable().alleles(siteNumber);
		byte major = myGenoPheno.genotypeTable().majorAllele(siteNumber);
		double[] coding = ModelEffectUtils.getNumericCodingForAdditiveModel(geno, major);
	...
	private void calculateBasis(List<ModelEffect> baseModel) {
		//assume no missing data
		//create othornormal basis for the base model
		ArrayList<DoubleMatrix> modelVectors = new ArrayList<>();
		ArrayList<ModelEffect> effects = new ArrayList<>(baseModel);
		...
		int n = basisVectors.get(0).numberOfRows();
